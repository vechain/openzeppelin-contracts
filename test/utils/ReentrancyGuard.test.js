...
contract('ReentrancyGuard', function () {
  ...
  it('does not allow remote callback', async function () {
    ...
    await expectRevert.unspecified(this.reentrancyMock.countAndCall(attacker.address));
  });
  ...
  it('does not allow local recursion', async function () {
    await expectRevert.unspecified(this.reentrancyMock.countLocalRecursive(10));
  });

  it('does not allow indirect local recursion', async function () {
    await expectRevert.unspecified(this.reentrancyMock.countThisRecursive(10));
  });
});
