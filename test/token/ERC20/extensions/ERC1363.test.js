...
async function fixture() {
  ...
  const receiverRaw = await ethers.deployContract('ERC1363ReceiverMock');
  const receiver = receiverRaw.attach(ethers.getAddress(await receiverRaw.getAddress()));
  const spenderRaw = await ethers.deployContract('ERC1363SpenderMock');
  const spender = spenderRaw.attach(ethers.getAddress(await spenderRaw.getAddress()));
  const tokenRaw = await ethers.deployContract('$ERC1363', [name, symbol]);
